...
export default function Claims({ parent_container }) {
  ...
  const getClaims = async () => {
    ...
    if (authClaims.agent) {
      const agentClaims = allClaims.filter(
        (userClaim) => userClaim.uid === authentication.currentUser.uid
      );
      ...
      listUsers()
        .then(({ data }) => {
          ...
          const supervisorClaims = allClaims.filter((userClaim) =>
            usersUnderSupervisor.includes(userClaim.uid)
          );
      ...
      listUsers()
        .then(({ data }) => {
          ...
          const adminClaims = allClaims.filter((userClaim) =>
            usersUnderAdmin.includes(userClaim.uid)
          );
  ...
  const handleDelete = async () => {
    ...
    await deleteDoc(claimDoc)
      .then(async () => {
        toast.success(
          `Successfully deleted ${singleDoc.claimantName}'s claim`,
          { position: "top-center" }
        );
      })
      .then(async () => {
      ...
      .catch(async () => {
        toast.error(`Failed to deleted ${singleDoc.claimantName}'s claim`, {
          position: "top-center",
        });
        await addDoc(logCollectionRef, {
          timeCreated: `${new Date()
  ...
  const searchByName = (data) =>
    data.filter(
      (row) =>
        row.claimantName.toLowerCase().indexOf(searchText.toLowerCase()) > -1
    );
  ...
  const shownClaims =
    ...
    currentClaims.filter(
      (claim) => !switchCategory || claim.status === switchCategory
    );

  const paginatedShownClaim =
    !claims || shownClaims.slice(indexOfFirstClaim, indexOfLastClaim);
    ...
    <div className="components">
      ...
      <div className={openToggle ? "myModal is-active" : "myModal"}>
        <div className="modal__content wack">
          <h1 className="wack">Delete?</h1>
          <p className="wack">
            Are you sure you want to delete <b>{singleDoc.claimantName}</b>'s
            claim?
          </p>
      ...
      <Modal show={showNotification} onHide={handleCloseNotification}>
        <ClaimModelNotification
          singleDoc={singleDoc}
          handleClose={handleClose}
          changeClaimStatus={changeClaimStatus}
        />
      </Modal>
