...
function Logs({ parent_container }) {
  ...
  const getLogs = async () => {
    ...
    const allAttendence = data.docs
      .map((doc) => ({ ...doc.data(), id: doc.id }))
      .filter((attendance) => attendance.uid === authentication.currentUser.uid)
      .filter((attendence) => attendence.checkin)
      .sort(
        (a, b) =>
          convertStringToDate(b.checkin) - convertStringToDate(a.checkin)
      );
    ...
    <div className="components">
      ...
      <div id="add_client_group">
        {attendence !== null && attendence.length > 0 ? (
          ...
          todayAttendence.checkin.slice(0, 10) ? (
            <button
              onClick={() => {
                submitLogin();
              }}
            ...
            todayAttendence?.checkout.slice(0, 10) ? (
            <button
              onClick={() => {
                submitLogout();
              }}
