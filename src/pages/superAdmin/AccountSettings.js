...
function AccountSettings() {
  ...
  useEffect(() => {
    document.title = `Settings ${systemName ? "- " + systemName : ""}`;
    downloadFavicon();
  ...
  const downloadFavicon = () => {
    ...
    getDownloadURL(ref(storage, "icons/favicon.ico")).then((url) => {
      console.log(url);
    });
  };
    ...
    <div className="components">
      ...
      >
        <div className="tw-py-2 tw-px-3">
          <h2 className="tw-text-lg tw-font-medium">Change System Name</h2>
          {/* <h2>SWICO</h2> */}
          <div className="tw-mb-1 tw-mt-3 tw-outline tw-outline-1 tw-outline-black tw-rounded-md">
            <input
              type="text"
              onChange={(event) => setNewName(event.target.value)}
              className="tw-py-2 tw-px-3  tw-appearance-none tw-w-full"
          ...
          <button
            onClick={async () => {
              if (newName) {
                setSystemName(newName.toUpperCase());
        ...
        <h2 className="tw-text-lg tw-font-medium tw-px-2 tw-mt-5 tw-py-2 tw-border-gray-500 tw-border-b-[1px]">
        ...
        <Formik
          ...
          onSubmit={async ({ logoURL, faviconURL, logoSm }, resetForm) => {
            ...
            if (faviconURL) {
              uploadString(faviconRef, faviconURL, "data_url")
                .then(({ ref }) => {
                  ...
                  if (ref) {
                    getDownloadURL(ref).then((url) => setFavicon(() => url));
                  }
                })
                .catch((error) => {
                  console.log(error);
            ...
            if (logoURL) {
              uploadString(logoRef, logoURL, "data_url")
                .then(({ ref }) => {
                  if (ref) {
                    getDownloadURL(ref).then((url) => setLogo(() => url));
                  }
                })
                .catch((error) => {
                  console.log(error);
            ...
            if (logoSm) {
              uploadString(smallLogoRef, logoSm, "data_url")
                .then(({ ref }) => {
                  if (ref) {
                    getDownloadURL(ref).then((url) => setLogoSm(() => url));
                  }
                })
                .catch((error) => {
                  console.log(error);
          ...
          validationSchema={Yup.object().shape({
            logoURL: Yup.mixed()
              .test(
                "either_logo",
                "At least one of the icons is required",
                (value, { createError, parent: { faviconURL, logoSm } }) => {
                  return !faviconURL && !value && !logoSm
                    ? createError({
                        message: "At least one of the icons is required.",
                      })
                    : true;
                }
              )
                ...
                async (value, { createError }) => {
                  if (value?.length > 0) {
                    ...
                    if (width > 512 || height > 250) {
                      ...
                      return createError({
                        message:
                          "Width and height should not exceed 512px and 250px respectively",
                      });
                    } else if (width > 512) {
                      return createError({
                        message: "Width should not exceed 512px",
                      });
                    } else if (height > 250) {
                      return createError({
                        message: "Height should not exceed 250px",
                      });
                    } else if (atob(value.substr(22)).length * 0.000001 > 2) {
                      return createError({
                        message: "Logo should not exceed 2MBs",
                      });
                    } else {
                      return true;
                    }
            ...
            faviconURL: Yup.mixed()
              .test(
                "either_favicon",
                "The width and height should be equal",
                (value, { createError, parent: { logoURL, logoSm } }) => {
                  return !logoURL && !value && !logoSm
                    ? createError({
                        message: "At least one of the icons is required.",
                      })
                    : true;
              ...
              .test(
                ...
                async (value, { createError }) => {
                  if (value?.length > 0) {
                    ...
                    if (width !== height) {
                      return createError({
                        message: "The height and width shoud be the same",
                      });
                    } else if (width > 512) {
                      return createError({
                        message: "Width should not exceed 512px",
                      ...
                      return createError({
                        message: "Favicon should not exceed 2MBs",
                      });
                    } else {
                      return true;
                    }
            ...
            logoSm: Yup.mixed()
              .test(
                "either_favicon",
                "The width and height should be equal",
                (value, { createError, parent: { logoURL, faviconURL } }) => {
                  return !logoURL && !value && !faviconURL
                    ? createError({
                        message: "At least one of the icons is required.",
                      })
                    : true;
                ...
                async (value, { createError }) => {
                  if (value?.length > 0) {
                    ...
                    if (width !== height) {
                      ...
                      return createError({
                        message: "Small Logo should not exceed 2MBs",
                      });
                    } else {
                      return true;
                    }
